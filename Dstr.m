function [route,numExpanded] = Dstr (input_map, start_coords, dest_coords)
...
for i = 1:12
    for j = 1:12
        if map1(i,j) == 0
            if map1(i+1,j)==999 || map1(i+1,j+1)==999 || map1(i,j+1)==999||map1(i-1,j+1)==999||map1(i+1,j-1)==999||map1(i-1,j)==999||map1(i,j-1)==999||map1(i-1,j-1)==999
                map1(i,j)=-1;
                continue;
            end           
        end
...
for h = 1:1
    for i = 1:12
        for j = 1:12
            if map1(i,j) == 0
                if map1(i+1,j)==-h || map1(i+1,j+1)==-h || map1(i,j+1)==-h||map1(i-1,j+1)==-h||map1(i+1,j-1)==-h||map1(i-1,j)==-h||map1(i,j-1)==-h||map1(i-1,j-1)==-h
                    map1(i,j)=-h-30;
                    continue;
                end           
            end
